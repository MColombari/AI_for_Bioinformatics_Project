...
class LPD:
    ...
    def preprocessing(self):
                ...
                for root, dirs, files in os.walk(self.folder_gene_path + "/" + dir):
                    for file in files:
                        if file in file_to_case_id.keys():
                            ...
                            parsed_file = parsed_file.fillna(0)
        ...
        for case_index in range(self.datastructure.shape[0]):
            self.list_df_CNV_filtered.append(self.datastructure['values'].loc[case_index][self.datastructure['values'].loc[case_index]['gene_id'].isin(gene_significativi)])
