setwd("/Users/davidelupo/Desktop/GGNN-main/data")
# load kegg
# install.packages("openxlsx", repos='https://cloud.r-project.org/')
library(openxlsx)
kegg<-read.xlsx("c2.cp.kegg.v5.2.symbols.xlsx",colNames = FALSE,rowNames = TRUE)
kegg<-kegg[,-1] # remove website 
kegg_genes<-c(t(kegg)) # convert into vector
kegg_genes<-na.omit(kegg_genes) # remove Nan
kegg_genes<-unique(kegg_genes) # keep unique genes

# load hprd -> Human Protein Reference Database
...
s<-colSums(adj_inter)
inter_genes<-inter_genes[s>0] # keep only genes with at least one connection

# data set
setwd("/Users/davidelupo/Desktop/GGNN-main/data/ov_tcga")
...
plist<-Reduce(intersect,list(patient_list,p1,p2,p3)) # Trova i pazienti che sono presenti in tutti e 4 i dataset: clinici, CNA, RNA, e metilazione
CNA<-CNA[,plist]
RNA<-RNA[,plist]
